...
def process(fin, fon, iden, version, evalue, prob, minCoverage, pipeline, version_m):
	...
	path+"database/"+version_m+"/features.gene.length")} 
    ...
    fo = open(fon+'.TF', 'w')  # second parameter is the output file
    fo2 = open(fon+'.potential.TF', 'w')
    fo.write("#TF\tquery-start\tquery-end\tread_id\tpredicted_TF-class\tbest-hit\tprobability\tidentity\talignment-length\talignment-bitscore\talignment-evalue\tcounts\n")
    fo2.write("#Tf\tquery-start\tquery-end\tread_id\tpredicted_TF-class\tbest-hit\tprobability\tidentity\talignment-length\talignment-bitscore\talignment-evalue\tcounts\n")
